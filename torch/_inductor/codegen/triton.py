...
class TensorDescriptorOptions(BlockDescriptorOptions):
    ...
    def create(
        ...
        *,
        ...
        """Helper to create a BlockDescriptorOptions instance"""
        # dummy_dim_block_expr must be >= 2, otherwise it will be removed if it is 1
        # in the constructor of BlockDescriptorOptions
        dummy_dim_block_expr = 1 + sympy.Symbol("DUMMY_DIM_BLOCK", integer=True, positive=True)
        inserted_dummy_dim = False
        if issubclass(stride_sorter_cls, BlockParameters.TensorDecriptorStrideSorter):
            has_stride_one_dim = False
            for i in range(len(params.strides)):
                if V.graph.sizevars.statically_known_equals(params.strides[i], sympy.Integer(1)) and not V.graph.sizevars.statically_known_equals(params.block_shape[i], sympy.Integer(1)):
                    has_stride_one_dim = True
            if not has_stride_one_dim:
                params += BlockParameters(shape=[sympy.Integer(1)], block_shape=[dummy_dim_block_expr], strides=[sympy.Integer(1)], offsets=[sympy.Integer(0)])
                inserted_dummy_dim = True
        ...
        if inserted_dummy_dim:
            assert options.params.block_shape[-1] == dummy_dim_block_expr
            options.params.block_shape[-1] = sympy.Integer(1)
            for i, shape in enumerate(options.broadcast_shape):
                if shape == dummy_dim_block_expr:
                    options.broadcast_shape[i] = sympy.Integer(1)
